...
if ($method === 'POST') {
    ...
    if ($action === 'add') {
        ...
        // Aturan berbeda per type
        if ($type === 'ranked') {
            // Ranked: hanya butuh tanggal
            if ($matchDate === '') {
                $matchDate = date('Y-m-d');
            }
            $matchTime = '00:00:00'; // fallback agar determineStatus tidak error
        } elseif ($type === 'scrim') {
            // Scrim: wajib lawan & tanggal, tidak butuh jam
            if ($opponentName === '' || $matchDate === '') {
                http_response_code(400);
                echo json_encode(['ok' => false, 'message' => 'Lawan dan tanggal wajib diisi untuk Scrim']);
                exit;
            }
            $matchTime = '00:00:00'; // fallback
