...
(function () {
  ...
  function loadCompetitions(selectedId) {
    ...
    fetch(`${apiBase}competition_api.php?action=list`)
      ...
      .catch((err) => {
        showToast(err.message || 'Gagal memuat daftar kompetisi.', 'error');
      });
  }
  ...
  function populateForm(match) {
    ...
    // Populate setelah visibilitas diset
    ...
    if (timeInput) {
      // Potong ke HH:MM agar cocok dengan input type="time"
    ...
    if (matchFormatSelect && match.match_format) {
      matchFormatSelect.value = match.match_format;
    }
    ...
    if (opponentScoreInput) opponentScoreInput.value = match.opponent_score != null ? match.opponent_score : 0;

    const statusSelect = document.getElementById('matchStatus');
    if (statusSelect) statusSelect.value = match.status || 'upcoming';

    // Competition dropdown sudah di-load dengan selectedId di loadCompetitions
    if ((type === 'tournament' || type === 'league') && match.competition_id) {
      loadCompetitions(match.competition_id);
    }

    // Update breadcrumb & title
    const label = match.opponent_name
      ? `Edit Match vs ${match.opponent_name}`
      : 'Edit Match';
  ...
  function validatePayload(payload) {
    ...
    } else if (payload.type === 'ranked') {
      if (!payload.match_date) return { valid: false, message: 'Tanggal wajib diisi untuk Ranked' };
    }
  ...
  function handleSubmit(event) {
    ...
    const payload = {
      ...
      match_format:    formData.get('matchFormat') || null,
    ...
    fetch(`${apiBase}match_api.php`, {
      ...
      .then(async (res) => {
        ...
        if (!res.ok || !json || !json.ok) {
          throw new Error((json && json.message) || 'Gagal memperbarui match.');
        }
        return json.match;
      })
      .then(() => {
      ...
      .catch((err) => {
        showToast(err.message || 'Terjadi kesalahan saat memperbarui match.', 'error');
      });
  }
