...
(function () {
  ...
  function populateForm(competition) {
    ...
    for (let i = 1; i <= 4; i += 1) {
      ...
      const finalRankField = document.getElementById(`finalRank${i}`);
      const resultField = document.getElementById(`result${i}`);
      const teamCountField = document.getElementById(`phase${i}TeamCount`);
      const startDateField = document.getElementById(`phase${i}StartDate`);
      ...
      if (finalRankField) finalRankField.value = competition[`phase_final_rank${i}`] || '';
      if (resultField) resultField.value =
        competition[`phase_result${i}`] != null ? competition[`phase_result${i}`] : '';
      if (teamCountField) teamCountField.value =
        competition[`phase_team_count${i}`] != null ? competition[`phase_team_count${i}`] : '';
      if (startDateField) startDateField.value = competition[`phase_start_date${i}`] || '';
    }
